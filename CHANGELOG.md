...
- Fixed `server start` daemon mode dropping startup options (model, config, shell-policy, search keys, TLS).
- Fixed JSON mode to emit structured errors instead of ANSI-colored stderr.
- Fixed quiet mode to output compact script-friendly lines instead of dashboards/tables.
