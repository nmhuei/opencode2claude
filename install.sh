...
print_welcome() {
    ...
    if command -v opencode >/dev/null 2>&1; then
        ...
        printf '%s\n' ""
        printf '%s\n' "  Start the bridge manually first; bare opencode2api then loads the bridge environment for Claude Code and opens it."
        ...
        printf '     %s%s%s\n' "${CYAN}" "curl -fsSL https://docs.opencode.ai/install.sh | sh" "${NC}"
        printf '%s\n' ""
        printf '%s\n' "  2. Then open Claude Code through the bridge:"
        printf '     %s%s%s\n' "${CYAN}" "opencode2api" "${NC}"
    fi
    printf '%s\n' ""
    printf '  %s%s%s\n' "${BOLD}" "Resources" "${NC}"
