...
| **Expected behavior contract** | CI calls `verify.sh all --profile ci` as source of truth. Release workflow uses `--locked` for reproducible builds. Release produces Linux binaries for linux-amd64 and linux-arm64 only. Dockerfile uses `--locked`. Version consistent across Cargo.toml and CHANGELOG.md. install.sh present. No 40010 references. |
